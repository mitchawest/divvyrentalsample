...
const errorHandler: IdentifiedErrorHandler = (err: {[key: string]: string }, req, res, next) => {
  logger.debug(err);
  ...
  if (errorCode && statusCodes.includes(errorCode.toString())) {
    logger.error(`Request error | Method: ${req.method} | Path: ${req.path} | Identifier: ${req.requestId} | Message: ${errorMessage}`);
    res.status(errorCode).send(new CustomErrorResponse(errorCode, req.requestId, req.path, errorMessage || ERROR_CODES[errorCode], err).getErrorResponse());
    next();
    return;
  }
  ...
  if (errorMessage) {
    ...
    statusMessages.forEach((message) => {
      if (errorMessage.toUpperCase().includes(message.toUpperCase()) && isSent === false) {
        isSent = true;
        logger.error(`Request error | Method: ${req.method} | Path: ${req.path} | Identifier: ${req.requestId} | Message: ${errorMessage}`);
        const status = Object.keys(ERROR_CODES).find((key) => ERROR_CODES[Number(key)] === message);
        res.status(Number(status)).send(new CustomErrorResponse(Number(status), req.requestId, req.path, errorMessage, err));
      }
    });
    if (isSent) return;
  }
  /* If response not yet identified, send server error */
  logger.error(`Request error | Method: ${req.method} | Path: ${req.path} | Identifier: ${req.requestId} | Message: ${errorMessage || 'Internal Server Error'}`);
  res.status(500).send(new CustomErrorResponse(500, req.requestId, req.path, errorMessage || ERROR_CODES[500], err).getErrorResponse());
  next();
};
